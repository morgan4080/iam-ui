<script setup lang="ts">
  ...
  const refresh = async () => {
    try {
      ...
      const query = ref(<string>`?${filterForm.searchTerm !== '' ? 'searchTerm=' + filterForm.searchTerm + '&': ''}order=${filterForm.order}&sort=${filterForm.order}&pageSize=${filterForm.recordsPerPage}&pageIndex=${currentPage.value - 1}`)
