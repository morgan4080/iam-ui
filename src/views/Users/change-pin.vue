...
const validatePin = async (e: any) => {
  ...
  if (e.target.value.length > 4) {
    e.target.value = e.target.value.slice(0,-1)
    e.target.classList.add('focus:ring-red-400')
    e.target.classList.add('focus:border-red-300')
    e.target.onblur = () => {
      e.target.classList.remove('focus:ring-red-400')
      e.target.classList.remove('focus:border-red-400')
    }
            ...
            <div class="py-6 px-4 sm:p-6">
              ...
              <div class="mt-6 sm:mt-5 space-y-6 sm:space-y-5">
                ...
                <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                  ...
                  <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class="max-w-lg flex rounded-md shadow-sm">
                      <input @input="validatePin($event)" v-model="form.pinConfirmation" type="password" name="pin-confirmation" pattern="[0-9]{4,4}" id="pin-confirmation" autocomplete="pin-confirmation" class="flex-1 block w-full focus:ring-indigo-500 focus:border-indigo-500 min-w-0 rounded-md sm:text-sm border-gray-300" required>
                    </div>
                  </div>
                </div>
              </div>
